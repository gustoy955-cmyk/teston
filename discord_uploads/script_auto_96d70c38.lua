...
local function ConstruirInterface(tema)
    ...
    -- === A NOVA LÓGICA DE FUSÃO PERFEITA QUE VOCÊ PEDIU === --
    Main:CreateToggle({
        Name = "🔗 Auto Fusão Definitiva (1 Segundo)",
        CurrentValue = false,
        Flag = "ToggleMergeNovo",
        Callback = function(Value)
            _G.AutoMergeNova = Value
            task.spawn(function()
                while _G.AutoMergeNova do
                    local myBase = GetPlayerBase()
                    local char = LocalPlayer.Character
                    local root = char and char:FindFirstChild("HumanoidRootPart")
                    ...
                    if myBase and root then
                        ...
                        if #nukeList > 0 then
                            local mergeSpot = root.CFrame -- Onde o jogador está virará o ponto de fusão
                            
                            -- PASSO 1: O Arrastão (Puxar todas pro mesmo lugar)
                            for _, nuke in ipairs(nukeList) do
                                if not _G.AutoMergeNova then break end
                                if nuke.Parent then
                                    TeleportTo(nuke)
                                    task.wait(0.1)
                                    ReplicatedStorage.NukeRemotes.PickUp:FireServer(nuke)
                                    task.wait(0.1)
                                    ReplicatedStorage.NukeRemotes.Drop:FireServer(mergeSpot)
                                    task.wait(0.1)
                                end
                            end
                            
                            -- Volta para o centro do monte
                            if root then root.CFrame = mergeSpot end
                            task.wait(0.3)
                            
                            -- PASSO 2: Pegar 1 por 1 sem repetir e segurar
                            for _, nuke in ipairs(nukeList) do
                                if not _G.AutoMergeNova then break end
                                -- Verifica se a bomba não foi destruída numa fusão anterior
                                if nuke.Parent then 
                                    ReplicatedStorage.NukeRemotes.PickUp:FireServer(nuke)
                                    task.wait(1.2) -- O SEGREDO: Segura por 1.2 segundos para fundir
                                    ReplicatedStorage.NukeRemotes.Drop:FireServer(mergeSpot)
                                    task.wait(0.1) -- Solta para não bugar a mão
                                end
                            end
                        end
                    end
                    task.wait(0.5) -- Terminou a fila? Aguarda e recomeça o ciclo!
                end
            end)
        end
    })
