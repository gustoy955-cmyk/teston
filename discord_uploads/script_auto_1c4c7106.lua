-- [[ AUTO CAÇADOR V4 (Raio-X de Corpo + Foco no Mapa Real) ]] --
...
Title.BackgroundColor3 = Color3.fromRGB(80, 0, 30)
Title.Text = "🎯 CAÇADOR V4 (MAPA REAL)"
...
-- ==========================================
-- LÓGICA DE CAÇA (SÓ NO MAPA REAL)
-- ==========================================
local function GetClosestPet(petName)
    ...
    -- Focando APENAS no RuntimeCacheServer para ignorar os bichos da cena de batalha
    local ServerCache = Workspace:FindFirstChild("RuntimeCache")
    ...
    -- Vasculha as pastas numeradas do servidor
    for _, folder in ipairs(ServerCache:GetChildren()) do
        for _, pet in ipairs(folder:GetChildren()) do
            
            -- Se tiver nome, filtra. (Ignora letras maiúsculas/minúsculas)
            if petName ~= "" and string.lower(pet.Name) ~= string.lower(petName) then
                continue
            end
            
            -- O NOVO RAIO-X: Procura de qualquer jeito uma parte física para ir até ela
            local targetPart = pet.PrimaryPart 
                or pet:FindFirstChild("HumanoidRootPart")
                or pet:FindFirstChild("InteractionPart") -- Os Bosses têm essa parte
                or pet:FindFirstChild("Torso")
                or pet:FindFirstChildWhichIsA("MeshPart")
