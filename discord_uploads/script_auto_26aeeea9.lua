-- [[ AUTO CAÇADOR V3 (Visão Dupla Cliente/Servidor) - Mobile Edition ]] --
...
Title.BackgroundColor3 = Color3.fromRGB(80, 50, 0)
Title.Text = "🎯 CAÇADOR V3"
...
-- ==========================================
-- LÓGICA DE CAÇA (CLIENTE + SERVIDOR)
-- ==========================================
local function GetClosestPet(petName)
    ...
    if not RuntimeCache then return nil, nil end

    -- Criamos uma lista para olhar nas duas pastas!
    local pastasParaOlhar = {}
    
    local ServerCache = RuntimeCache:FindFirstChild("RuntimeCacheServer")
    if ServerCache and ServerCache:FindFirstChild("CreatureModelCache") then
        table.insert(pastasParaOlhar, ServerCache.CreatureModelCache)
    end
    
    local ClientCache = RuntimeCache:FindFirstChild("RuntimeCacheClient")
    if ClientCache and ClientCache:FindFirstChild("CreatureModelCache") then
        table.insert(pastasParaOlhar, ClientCache.CreatureModelCache)
    end
    ...
    -- Vasculha as duas pastas
    for _, cacheFolder in ipairs(pastasParaOlhar) do
